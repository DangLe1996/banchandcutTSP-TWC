	...
	double* earliest, double* latest)
{
	...
	//	rhs[index1] = 1;
	//	matbeg[index1++] = index;
	//	matind[index] = index_e[from[i]][to[i]]; //add y_{ij} variable
	//	matval[index++] =1;
	//	
	//}
	//if (addValidation)
	...
	for (i = 0; i < N; i++) {
		...
		matbeg[index1++] = index;
		matind[index] = E + Np + i; //add E_i variable
		matval[index++] = 1;
		matind[index] = E + i; //add w_i variable
		matval[index++] = 1;
	}
	...
	for (i = 0; i < N; i++) {
		...
		rhs[index1] = -latest[i];
		matbeg[index1++] = index;
		matind[index] = E + Np + N + i; //add T_i variable
		matval[index++] = 1;
		matind[index] = E + i; //add w_i variable
		matval[index++] = -1;
	}
	...
	CPXsetintparam(env, CPXPARAM_ClockType, 2); //clock type, 1 is CPU and 2 is Wall Clock
   //CPXsetintparam(env,CPX_PARAM_INTSOLLIM,1);    //stops after finding first integer sol.
	...
	CPXsetdblparam(env, CPXPARAM_RandomSeed, 10); // random seed
	...
	CPXsetintparam(env, CPX_PARAM_MIPSEARCH, CPX_MIPSEARCH_TRADITIONAL);
	//CPXsetintparam(env, CPXPARAM_MIP_SubMIP_NodeLimit, 100);
...
int        *useraction_p)
{
	...
	*useraction_p = CPX_CALLBACK_DEFAULT;
	...
	//status = CPXwriteprob(env, nodelp, "modelTSP-2.lp", NULL);
