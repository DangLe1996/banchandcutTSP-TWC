...
EXPORT double solve_Clustered_ATSP(int depot, int dim_N, int dim_Z, double **c, int *zone_dim, int *list_all_stops, int *final_sequence, double lambda, double delta, double **M, double *W, double *earliest, double *latest)
{
	int i,j, e, t, z, zp, Z, count_stops;
	int index, index1;  // auxiliar indices to fill in the constraint matrix
	double best_upper_bound, best_lower_bound, check_ub, check_ub2;
	//int nodecount;
	...
	double    value, value2;   // objevtive value of solution ..................................
	//int* sec_zone;
	ZONEID* zoneid;
	...
	N = dim_N; //get it as argument
	Np = N + 1;
	//E = (Np - 1)*(Np - 1); // compute it here
	//E = Np * Np - 2 * N;
	e = 0;
	for (i = 0; i < N; i++) {
		for (j = 1; j < Np; j++) {
	...
	index_e = create_int_matrix(Np,Np);
	sepcut.cutind = create_int_vector(E);
	sepcut.cutval = create_double_vector(E);
	cutset = (CUTSET *)calloc(Np - 1, sizeof(CUTSET));
	for (i = 0; i < Np - 1; i++)
		cutset[i].S = create_int_vector(Np);
	capacity = create_double_matrix(Np, Np);
	visited = create_int_vector(Np);
	queue = create_int_vector(Np);
	tree = create_int_vector(Np);
	currentFlow = create_double_matrix(Np, Np);
	previous = create_int_vector(Np);
	alpha = create_double_vector(E);
	//sec_zone = create_int_vector(Z+1);
	zoneid = (ZONEID*) calloc(Z + 1, sizeof(ZONEID));
	for (z = 0; z < Z + 1; z++)
		zoneid[z].stops = create_int_vector(Np);

	e = 0;
	for (i = 0; i < N; i++) {
		for (j = 1; j < Np; j++) {
			if (i != j) {
				index_i[e] = i; //compute it here
				index_j[e] = j;
				index_e[i][j] = e;
				e++;
			}
	...
	//assert(e == E);
	//printf("flag1b \n");
	// 	   This is how to set the zoneid sequencing decisions acording to observed route
	//for (z = 0; z < Z + 1; z++) {
	//	zoneid[z].dim = route_HQ[s].zoneid[z].dim; //get is as argument
	//	//sec_zone[z] = route_HQ[s].sec_zone_obs[z];
	//	for (i = 0; i < route_HQ[s].zoneid[z].dim; i++)
	//		zoneid[z].stops[i] = route_HQ[s].zoneid[z].stops[i]; //get is as argument
	//}
	count_stops = 0;
	for (z = 0; z < Z + 1; z++) {
	...
	// 	   This is how to set the zoneid sequencing decisions acording to our "optimal" sequence from the SDTSP
	/*for (z = 0; z < Z; z++) {
		for (t = 0; t < Z; t++) {
			if (route_HQ[s].zoneid[t].uzoneid == route_HQ[s].sec_zone_opt[z]) {
				zoneid[z].dim = route_HQ[s].zoneid[t].dim;
				zoneid[z].stops = create_int_vector(zoneid[z].dim);
				for (i = 0; i < zoneid[z].dim; i++)
					zoneid[z].stops[i] = route_HQ[s].zoneid[t].stops[i];
				break;
			}
		}
	}
	zoneid[Z].dim = 1;
	zoneid[Z].stops = create_int_vector(zoneid[Z].dim);
	zoneid[Z].uzoneid = zoneid[0].uzoneid;
	zoneid[Z].stops[0] = zoneid[0].stops[0];*/
	//printf("flag1c \n");
	...
	/*for (e = 0; e < E; e++)
		index_e[i][j] = route_HQ[s].index_a[i][j];*/
	...
	for (e = 0; e<E; e++){
		//pos_y[e] = index1;
		obj[e] = c[index_i[e]][index_j[e]]; //get it as argument
		lb[e] = 0;
		ub[e] = 1;
		ctype[e] = 'B';
		//sprintf(colname[e], "y_%d,%d ", index_i[e], index_j[e]);
	}
	status = CPXnewcols(env, lp, E, obj, lb, ub, ctype, NULL);
	if (status)
	...
	//Define w_i variables (service start time)
	numcols = Np;
	/*colname = (char**)calloc(numcols, sizeof(char*));
	for (i = 0; i < numcols; i++){
		colname[i] = (char*)calloc(255, sizeof(char));
	...
	for (i = 0; i < N; i++) {
		//pos_y[e] = index1;
		obj[i] = lambda; //get it as argument
		lb[i] = 0;
		//ub[i] = CPXinfty;
		ctype[i] = 'C';
		//sprintf(colname[e], "y_%d,%d ", index_i[e], index_j[e]);
	...
	for (i = 0; i < N; i++) {
		//pos_y[e] = index1;
		obj[i] = delta; //get it as argument
		lb[i] = 0;
		//ub[i] = CPXinfty;
		ctype[i] = 'C';
		//sprintf(colname[e], "y_%d,%d ", index_i[e], index_j[e]);
	...
	// Add zoneid transition constraints (There must be exactly one arc on the cut between two consecutive blocks of zoneid's)
	numrows = Z;
	numnz = 0;
	for (z = 0; z < Z; z++) {
		for (i = 0; i < zoneid[z].dim; i++) {
			for (j = 0; j < zoneid[z + 1].dim; j++) {
				//if (i != j) {
					numnz++;
				//}
			}
	...
	for (z = 0; z < Z; z++) {
		...
		for (i = 0; i < zoneid[z].dim; i++) {
			for (j = 0; j < zoneid[z + 1].dim; j++) {
				//if (i != j) {
					matind[index] = index_e[zoneid[z].stops[i]][zoneid[z + 1].stops[j]];
					matval[index++] = 1;
				//}
			}
		}
	...
	// Add zoneid transition constraints (Remove all arcs on the cut between a block and the complement of its next adjacent block)
	...
	// for (z = 1; z < Z; z++) {
		...
		// rhs[index1] = 0;
		// matbeg[index1++] = index;
		// for (i = 0; i < zoneid[z].dim-1; i++) {
			// for (zp = 0; zp < Z; zp++) {
				// if (zp != z && zp != (z + 1)) {
					// for (j = 0; j < zoneid[zp].dim; j++) {
						// if (i != j) {
						// matind[index] = index_e[zoneid[z].stops[i]][zoneid[zp].stops[j]];
						// matval[index++] = 1;
					// }
				// }
			// }
	...
	// Add one-block zoneid constraints (There must be exactly zoneid[z].dim-1 arcs all stops of each zoneid's)
	numrows = Z;
	numnz = 0;
	for (z = 0; z < Z; z++) {
		if (zoneid[z].dim > 1) {
			for (i = 0; i < zoneid[z].dim; i++) {
				for (j = 0; j < zoneid[z].dim; j++) {
					if (i != j) {
						numnz++;
					}
				}
	...
	for (z = 0; z < Z; z++) {
		if (zoneid[z].dim > 1) {
			sense[index1] = 'E';
			rhs[index1] = (zoneid[z].dim - 1);
			matbeg[index1++] = index;
			for (i = 0; i < zoneid[z].dim; i++) {
				for (j = 0; j < zoneid[z].dim; j++) {
					if (i != j) {
						matind[index] = index_e[zoneid[z].stops[i]][zoneid[z].stops[j]];
						matval[index++] = 1;
					}
				}
			}
	...
	// Add start service time constraints (w_i + W_i + t_{ij} <= w_j + M_{ij}*(1-x_{ij})
	numrows = N*N;
	numnz = N*N*3;
	d_vector(&rhs, numrows, "open_cplex:2");
	c_vector(&sense, numrows, "open_cplex:3");
	...
	// Add Earliness constraints (E_i >= e_i - w_i )
	numrows = N;
	numnz = N * 2;
	d_vector(&rhs, numrows, "open_cplex:2");
	c_vector(&sense, numrows, "open_cplex:3");
	...
	// Add Tardiness constraints (T_i >= w_i - l_i)
	numrows = N;
	numnz = N * 2;
	d_vector(&rhs, numrows, "open_cplex:2");
	c_vector(&sense, numrows, "open_cplex:3");
	...
	CPXsetdblparam(env, CPX_PARAM_TILIM, 180); // time limit
	...
	CPXsetintparam(env,CPX_PARAM_HEURFREQ, 20); //heuristic frequency and intensisty 
	CPXsetintparam(env,CPX_PARAM_RINSHEUR, 20); //RINS heuristic frequency and intensisty 
	...
	status = CPXsetusercutcallbackfunc(env, mycutcallbackATSP, &cutinfo);
	status = CPXsetlazyconstraintcallbackfunc(env, mycutcallbackATSP, &cutinfo);
	...
	CPXgetbestobjval(env, lp, &value2);  //best lower bound in case the problem was not solved to optimality
	best_lower_bound = value2;
	//printf("LB: %.2f  ",value2);
	//printf("gap: %.2f ", (double) abs(value - value2) / (double) abs(value) *100);
	...
	//for (e = 0; e < E; e++){
	//	if (x[e] > 0.01) {
	//		//printf("y[%d][%d]= 1 ", index_i[e], index_j[e]);
	//		route_HQ[s].x_opt[e] = 1;
	//		check_ub += c[index_i[e]][index_j[e]];
	//		//check_ub2 += route_HQ[s].time[index_i[e]][index_j[e]];
	//	}
	//	else 
	//		route_HQ[s].x_opt[e] = 0; 
	//}
	//printf("\n");

	//This lines are for Ivan's code in VS
	//for (i = 0; i < N; i++)
	//	route_HQ[s].sec_opt[i] = -1;
	//t = 0;
	//route_HQ[s].sec_opt[t++] = route_HQ[s].depot;
	////printf("%d, ", route_HQ[s].sec_opt[t-1]);
	//while (t < N + 1) {
	//	for (j = 0; j < N; j++) {
	//		if (route_HQ[s].sec_opt[t - 1] != j && x[index_e[route_HQ[s].sec_opt[t - 1]][j]]  > 0.01) {
	//			route_HQ[s].sec_opt[t++] = j;
	//			//printf("%d, ", route_HQ[s].sec_opt[t - 1]);
	//			break;
	//		}
	//	}
	...
	/*for (i = 0; i < route_HQ[s].dim; i++) {
		printf("stop: %d zone: %d \n", route_HQ[s].sec_opt[i], route_HQ[s].zone[route_HQ[s].sec_opt[i]]);
	}*/
	//Route_Refinement_Step(s);

	//These lines are for Dang's code in python
	for (i = 0; i < N; i++)
		final_sequence[i] = -1;
	t = 0;
	final_sequence[t++] = depot;
	//printf("%d, ", route_HQ[s].sec_opt[t-1]);
	while (t < N + 1) {
		for (j = 0; j < N; j++) {
			if (final_sequence[t - 1] != j && x[index_e[final_sequence[t - 1]][j]] > 0.01) {
				final_sequence[t++] = j;
				//printf("%d, ", route_HQ[s].sec_opt[t - 1]);
				break;
			}
	...
	/*for (i = 0; i < N + 1; i++)
		final_sequence[i] = route_HQ[s].sec_opt[i];*/


	//printf("\n");
	// 	  The following fucntion is needed for inverse model!!!!!
	//Compute_Constraint_Coefficients(s);


	/*route_HQ[s].a_same_zone_opt = 0;
	route_HQ[s].a_diff_zone_opt = 0;
	route_HQ[s].sumt_same_zone_opt = 0;
	route_HQ[s].sumt_diff_zone_opt = 0;
	for (i = 0; i < route_HQ[s].dim; i++) {
		if (route_HQ[s].zone[route_HQ[s].sec_opt[i]] == route_HQ[s].zone[route_HQ[s].sec_opt[i + 1]]) {
			route_HQ[s].a_same_zone_opt++;
			route_HQ[s].sumt_same_zone_opt += route_HQ[s].time[route_HQ[s].sec_opt[i]][route_HQ[s].sec_opt[i + 1]];
			check_ub2 += route_HQ[s].time[route_HQ[s].sec_opt[i]][route_HQ[s].sec_opt[i + 1]];
		}
		else {
			route_HQ[s].a_diff_zone_opt++;
			route_HQ[s].sumt_diff_zone_opt += route_HQ[s].time[route_HQ[s].sec_opt[i]][route_HQ[s].sec_opt[i + 1]];
			check_ub2 += route_HQ[s].time[route_HQ[s].sec_opt[i]][route_HQ[s].sec_opt[i + 1]];
		}
	}*/
	//route_HQ[s].sec_opt[route_HQ[s].dim] = route_HQ[s].sec_opt[0];

	/*printf("Optimal ATSP sequence: ");
	for (i = 0; i < N+1; i++)
		printf("%d ", route_HQ[s].sec_opt[i]);
	printf("\n");*/
	...
	//double** c, int* zone_dim, int* list_all_stops, int* final_sequence
	//free_double_matrix(c, N);
	//free(zone_dim);
	//free(list_all_stops);
...
mycutcallbackATSP(CPXCENVptr env,
	void* cbdata,
	int        wherefrom,
	void* cbhandle,
	int* useraction_p)
						...
						for (i = 0; i < cutset[n].dim; i++) {
							for (j = 0; j < cutset[n].dim; j++) {
								if (i != j) {
									...
									sepcut.cutval[sepcut.cutnz++] = 1;
									//printf("(%d,%d, ind[%d]) ",cutset[n].S[i], cutset[n].S[j], sepcut.cutnz - 1);
								}
							}
						}
						//printf("\n");
...
double min_cap_cuts(double** capacity) {
	...
	for (s = 1; s < Np; s++) {	// visit the other nodes and update 'tree'
